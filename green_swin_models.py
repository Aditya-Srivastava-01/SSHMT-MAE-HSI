...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        self.fc1 = nn.Linear(in_features, hidden_features)  #输入特征数为in_features，输出特征数为hidden_features,线性变换
        self.act = act_layer()                                #激活层，用gelu
        self.fc2 = nn.Linear(hidden_features, out_features)
        self.drop = nn.Dropout(drop)          #随机置零，drop是置零概率
...
class PConv2d(nn.Module):
    def __init__(self,
        ...
        self.dim_conv = in_channels // n_div                           #分割通道
        self.dim_untouched = in_channels - self.dim_conv             #部分割通道
    ...
    def forward_slicing(self, x: Tensor) -> Tensor:
        x[:, :self.dim_conv, :, :] = self.conv(x[:, :self.dim_conv, :, :])                 #对张量的前dim_conv维卷积操作
                                                 #这是对张量 x 的切片操作，选择了所有样本（:）的前 self.dim_conv 个通道（:self.dim_conv）以及所有高度和宽度位置（:, :），卷积完了后替代回原位置
        return x

    def forward_split_cat(self, x: Tensor) -> Tensor:
        x1, x2 = torch.split(x, [self.dim_conv, self.dim_untouched], dim=1)               #将张量进行分割操作，x1对应着要卷积的部分，x2对应着不卷积的部分
        x1 = self.conv(x1)                                                                                 #对x1进行卷积
        x = torch.cat((x1, x2), dim=1)                               #拼到一起

        return x                  #一般来说两种方法结果一样，但法一更节省内存，法二不会影响原张量
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, embed_dim=96, norm_layer=None):
        super().__init__()
        img_size = to_2tuple(img_size)           #转换为长度为2的元组（224，224）
        ...
        self.PConv = PConv2d(192)                     #192个通道

        # self.PConv = nn.Sequential(
        #     PConv2d(192),
        #     nn.Conv2d(in_channels=8 * 24, out_channels=256, kernel_size=(2, 2)),
        #     nn.BatchNorm2d(256),
        #     nn.ReLU(),
        # )
        ...
        self.conv3d_features = nn.Sequential(
            nn.Conv3d(1, out_channels=8, kernel_size=(7, 1, 1)),
            nn.BatchNorm3d(8),
            nn.ReLU(),
        )
        ...
        # self.dw_feature = DepthWiseConv(8*22, 256)

        # self.features_2D = FasterNetBlock(8 * 24,
        #                         bias = False,
        #                         act = 'ReLU',
        #                         n_div = 4,
        #                         forward = 'slicing',
        ...
        self.conv2d = nn.Sequential(
            nn.Conv2d(in_channels=in_chans, out_channels=30, kernel_size=(1, 1)),
            nn.BatchNorm2d(30),
            nn.ReLU(),
        )
...
class SwinTransformer(BaseGreenModel):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        self.pos_drop = nn.Dropout(p=drop_rate)     #随机元素归零

        # stochastic depth
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, sum(depths))]  # stochastic depth decay rule

        # fusion stage
        self.downsample = PatchMerging((4, 4), dim=128, norm_layer=nn.LayerNorm)
        # self.conv1d = nn.Sequential(
        #     nn.Conv1d(in_channels=2, out_channels=2, kernel_size=7),
        #     nn.BatchNorm1d(2),
        #     nn.GELU(),
        # )
        ...
        self.conv1d_2 = nn.Sequential(
            nn.Conv1d(in_channels=9, out_channels=8, kernel_size=1),
            nn.BatchNorm1d(8),
            nn.GELU(),
        )
        self.conv1d_3 = nn.Sequential(
            nn.Conv1d(in_channels=8, out_channels=2, kernel_size=1),
            nn.BatchNorm1d(2),
            nn.GELU(),
        )
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(dim=int(embed_dim * 2 ** i_layer) if (i_layer < self.num_layers - 3) else 256,
                               input_resolution=(patches_resolution[0] // (2 ** i_layer),
                                                 patches_resolution[1] // (2 ** i_layer)) if (i_layer < self.num_layers - 3) else (2, 2),
    ...
    def patchify(self, x):
        ...
        x = self.pos_drop(x)          #ape默认fasle，这里是dropout，元素随机归零
        return x
    
    def forward_features(self, x, mask):
        # patch embedding
        x = self.patchify(x)          #patchembed后随机归零

        # mask out some patches according to the random mask
        x_vis, coords, vis_mask = self.apply_mask(x, mask, self.patches_resolution)

        # fusion stage
        # x_f, c, v = self.downsample(x_vis, coords, vis_mask)
        ...
        for layer in self.layers:
            ...
            if layer.dim == 128:
                x_vis, coords, vis_mask = layer(x_vis, coords, vis_mask)
                x_f = x_vis
                x_f = self.conv1d_1(x_f)
            else:
                # x_f = x_vis
                x_vis, coords, vis_mask = layer(x_vis, coords, vis_mask)
                x_e = self.conv1d_1(x_vis)
                x_1 = torch.mean(x_f, dim=1, keepdim=True)
                x_2 = torch.mean(x_e, dim=1, keepdim=True)
                x_1 = torch.cat([x_1, x_e], dim=1)
                x_2 = torch.cat([x_2, x_f], dim=1)
                x_f = x_1 + x_2
                x_f = self.conv1d_2(x_f)
                # x_vis = x_f1 + x_f2
                # x_vis = self.conv1d(x_vis)
                # x_f1 = self.conv1d(x_f1)
                # x_f2 = self.conv1d(x_f2)
                # x_vis = x_f1 + x_f2

        x_f = self.conv1d_3(x_f)
...
if __name__ == "__main__":
    ...
    #
    # # the second last stagew
    # # mask_new = mask_new.reshape(H, 1, W, 1).repeat((1, 2, 1, 2)).reshape(-1)
    ...
    N = 50
    mask_ratio = 3 / 5.
    nvis, nmasked = int(N * (1 - mask_ratio)), int(N * mask_ratio)
    mask_new = torch.cat([torch.ones((nvis,)), torch.zeros((nmasked,))])
    mask_new = mask_new[torch.randperm(N)].bool()

    # the second last stage
    # mask_new = mask_new.reshape(H, 1, W, 1).repeat((1, 2, 1, 2)).reshape(-1)
    mask = mask_new.reshape(N, 1).repeat((1, 2)).reshape(-1)
    N = N * 2
    x_ori = torch.arange(N) * 100.
    x = x_ori[mask].reshape(1, nvis * 2, -1)
    B, L, C = x.shape
    print(mask.reshape(N))
    print(x_ori.reshape(N))

    # mask = mask.reshape(H//2, 2, W//2, 2).permute(0, 2, 1, 3).reshape(-1)

    mask = mask.reshape(N//2, 2).reshape(-1)
    coords = get_coordinates(10, 10).reshape(2, -1).permute(1, 0)

    # coords = get_coordinates(N).permute(1, 0)

    coords = coords.reshape(10//2, 2, 10//2, 2, 2).permute(0, 2, 1, 3, 4).reshape(-1, 2)
    coords_vis_local = coords[mask].reshape(-1, 2)
    coords_vis_local = coords_vis_local[:, 0] * H + coords_vis_local[:, 1]
    idx_shuffle = torch.argsort(torch.argsort(coords_vis_local))
