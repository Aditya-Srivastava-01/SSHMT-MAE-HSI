...
class BaseGreenModel(nn.Module):
    ...
    def apply_mask(self, x, mask, patches_resolution):          #mask是一个布尔张量，只有0或1，其中为零则是不可见，为1可见
        ...
        return x_vis, coords, vis_mask   #方法返回三个张量：x_vis 是可见补丁的数据，coords 是这些补丁的坐标，vis_mask 是一个布尔张量，表示哪些补丁是可见的。
...
class MaskedAutoencoder(nn.Module):
    ...
    def __init__(self, encoder, embed_dim, patch_size, in_chans=3,
                 ...
                 block_cls=Block, mlp_ratio=4, finetune=False,
                 num_classes=1000,
                 **kwargs):
        super().__init__()
        ...
        self.downsample = nn.Sequential(
            nn.Conv2d(in_channels=144, out_channels=144, kernel_size=(3, 3)),
            nn.BatchNorm2d(144),
            nn.ReLU(),
        )

        self.upsample = nn.ConvTranspose2d(in_channels=200, out_channels=200, kernel_size=3, padding=0)

        self.head = nn.Linear(embed_dim, num_classes) if num_classes > 0 else nn.Identity()
        # self.head = nn.Linear(16200, num_classes) if num_classes > 0 else nn.Identity()
    ...
    def patchify(self, imgs, patch_size=None):
        ...
        h = w = imgs.shape[2] // p
        x = imgs.reshape(shape=(imgs.shape[0], 200, h, p, w, p))
        x = torch.einsum('nchpwq->nhwpqc', x)
        x = x.reshape(shape=(imgs.shape[0], h * w, p ** 2 * 200))
        return x
    ...
    def forward(self, imgs, mask_ratio=0.5):
        ...
        if self.finetune:
            # latent = self.forward_decoder(latent, ids_restore)  # [N, L, p*p*3]
